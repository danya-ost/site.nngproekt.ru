...
$this->title = 'Редактирование элемента бара (нижний)'
?>
    ...
    <div class="mt-5">
        <label for="on_off_1" class="text-sm text-stone-500 cursor-pointer">
            <input type="radio"
                   name="on_off"
                   id="on_off_1"
                    <?= \frontend\models\Navbar::find()->where(['id' => $_GET['bbi']])->one()['status'] == 1 ? 'checked' : '' ?>
        ...
        <label for="on_off_0" class="text-sm text-stone-500 cursor-pointer ml-5">
            <input type="radio"
                   name="on_off"
                   id="on_off_0"
                    <?= \frontend\models\Navbar::find()->where(['id' => $_GET['bbi']])->one()['status'] == 0 ? 'checked' : '' ?>
...
$script = <<<JS
    $('#btn_save').on('click', function (){
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data: { 'id': '$id', 'title': $('input[name=\'title\']').val(), 'href': $('input[name=\'href\']').val(), 'on_off': $('input[name=\'on_off\']:checked').val() },
            beforeSend: function (){
                $('#btn_save').hide();
                $('#loader').show();  
            },
             success: function (){
                window.location.href = '/frontend/web/bar/edit-bottom';
            },
