...
/** @var \yii\web\View $data */

use yii\helpers\Url;

$this->title = 'Левый бар';
?>
                        ...
                        <?php foreach ( $data as $index => $item ): ?>
                            <tr class="border-b <?= \frontend\models\Navbar::find()->where(['id' => $item['navbar_id']])->one()['status'] == 0 ? 'bg-red-50' : 'hover:bg-stone-200' ?>">
                                <td class="text-sm px-4 py-1 whitespace-nowrap border-r"><span class="bg-red-50 hidden"></span>
                                    <a href="<?= $item['href'] ?>" class="w-[51px] h-[51px] bg-main-red-900 rounded-full flex items-center justify-center m-auto my-2 relative animation-data-title">
                                        <?= $item['svg'] ?>
                                    </a>
                                </td>
                                ...
                                <td class="text-sm px-4 py-1 whitespace-nowrap">
                                    <a href="<?= Url::to(['/bar/edit-left-item', 'lbi' => $item['navbar_id']]) ?>" class="text-xs text-stone-500 hover:text-black hover:font-bold block">
