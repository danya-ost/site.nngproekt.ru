...
/** @var \frontend\controllers\NewsController $user_avata */
...
<div class="container h-auto">
    ...
    <div class="w-full h-auto hidden sm:flex items-center justify-start">
        ...
        <i class="w-px h-5 bg-stone-300 block mx-2"></i>
        <a href="<?= $breadcrumbs[2] ?>" class="block font-light text-sm text-stone-500">
            <?= $data['title'] ?>
        </a>
    </div>

    <?php if ( $permission['updateBlog'] || $permission['deleteBlog'] ): ?>
        <div class="relative rounded overflow-hidden my-5 bg-white shadow-sm hover:shadow-lg px-2 py-3 border border-solid border-stone-200 duration-200 ease-in-out transition-all pb-10 sm:pb-3">
            ...
            <?php if ( $permission['updateBlog'] || $data['user_id'] == \frontend\tools\tools::idUser() ): ?>
                <a href="<?= Url::to(['/blog/update-form', 'b' => $data['id']]) ?>" class="block sm:inline-block align-middle text-center rounded px-6 py-2.5 ml-1 mt-1 md:mt-0 bg-stone-300 text-white font-medium text-xs leading-tight uppercase hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
            ...
            <?php endif; ?>
            <?php if ( $permission['deleteBlog'] ): ?>
                <button type="button" name="delete_news_<?= $data['id'] ?>" title="Удалить" class="block sm:inline-block w-full sm:w-auto align-middle text-center rounded px-6 py-2.5 ml-1 mt-1 md:mt-0 bg-stone-300 text-white font-medium text-xs leading-tight uppercase hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
    ...
    <div id="containerNews" class="max-w-[1100px] h-auto bg-white px-5 sm:px-14 md:px-24 py-8 mt-8 mb-8 m-auto">
        ...
        <?php if ( $data['video'] != NULL ): ?>
            <video src="/frontend/web/<?= $data['video'] ?>" controls class="w-full mt-5 mb-9 bg-no-repeat bg-center bg-cover">
                <source src="/frontend/web/<?= $data['video'] ?>" type="video/mp4">
            </video>
        <?php else: ?>
            <div class="w-full h-[350px] lg:h-[500px] mt-5 mb-9 bg-no-repeat bg-center bg-cover" style="background-image: url('/frontend/web/<?= $data['thumb'] ?>');"></div>
        <?php endif; ?>
    ...
    <div id="relevantPost" style="display: none;" class="w-full h-auto mb-16">
        <h1 class="font-bold text-4xl mb-9">
            ДРУГИЕ НОВОСТИ
        </h1>

        <div id="containerRelevantPost" class="w-full h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 grid-rows-3 sm:grid-cols-2 md:grid-rows-1 gap-5"></div>
    </div>
...
$script = <<<JS
    let e = {
        relevant_container: $('#containerRelevantPost'),
        loader_relevant: $('#relevantPost')
    }
    
    $.ajax({
        url: '/frontend/web/blog/views',
        type: 'post',
        dataType: 'html',
        data: { 'id': '$id' },
    ...
    $.ajax({
        url: '/frontend/web/blog/relevant-post',
        type: 'post',
        dataType: 'html',
        data: { 'id': '$id' },
    ...
    $('#btnDelete').on('click', function (){
        $.ajax({
            url: '/frontend/web/blog/delete-post',
            type: 'post',
            data: { 'id': '$id' },
            dataType: 'html',
            beforeSend: function (){
            ...
            success: function (response){
                window.location.href = '/frontend/web/blog/index';
            },
            error: function (response){
                alert("Упс! Кажется произошла ошибка при загрузке данных. Пожалуйста, перезагрузите страницу или попробуйте позже!\\n\\nОтвет ошибки:\\n" + response['responseText']);
                console.log(response);
    ...
    $.ajax({
        url: '/frontend/web/blog/view-comments',
        type: 'post',
        dataType: 'html',
        data: { 'id': '$id' },
        success: function (response){
    ...
    $('#sendComment').on('click', function (){
        ...
        if ( get_comment_type == 0 ){
            $.ajax({
                url: '/frontend/web/blog/add-comment',
                type: 'post',
                data: { 'comment': text_comment, 'id': '$id' },
                dataType: 'html',
                beforeSend: function (){
                    $('#sendComment').hide();
                    $('#loaderComment').show();
            ...
            $.ajax({
                url: '/frontend/web/blog/add-comment',
                type: 'post',
                data: { 'comment': text_comment, 'id': '$id', 'response': get_comment_id },
                dataType: 'html',
                beforeSend: function (){
                    $('#sendComment').hide();
                    $('#loaderComment').show();
    ...
    $('#comments').on('click', '#responseComment', function (){
        ...
        $.ajax({
            url: '/frontend/web/blog/view-user-news',
            type: 'post',
            dataType: 'html',
            data: { 'id': id_response },
            success: function (response){
    ...
    $('#comments').on('click', '#deleteComment', function (){
        $.ajax({
            url: '/frontend/web/blog/delete-comment',
            type: 'post',
            dataType: 'html',
