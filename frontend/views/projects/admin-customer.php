...
$this->title = 'Клиенты [Администрирование]'
?>

<div id="main-div" class="container h-auto">

    <div class="w-full ha-auto mb-6">
        <h1 class="font-bold text-4xl uppercase">Клиенты</h1>
    </div>
    ...
    <?php if ( $permission['addCust'] ) : ?>
        <button data-bs-toggle="modal" data-bs-target="#modalAddCust" class="inline-block align-middle rounded px-6 py-2.5 ml-1 bg-gray-800 text-white font-medium text-xs leading-tight uppercase shadow-md hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
            ...
            <span class="ml-1 inline-block align-middle">Добавить клиента</span>
        </button>
    <?php endif; ?>

    <div id="containerCust" class="w-full h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-5 mb-24"></div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable relative w-auto pointer-events-none">
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding outline-none text-current">
            ...
            <div id="modalContentAdd" class="modal-body relative p-4 cust-scroll">
                ...
                <label for="add-category" class="block font-light text-xs text-stone-500 mt-5">
                    Адрес клиента
                </label>
                <input type="text"
                       id="add-category"
                       name="new-cust-address"
            ...
            <div id="modelFooterAdd" class="modal-footer flex flex-shrink-0 flex-wrap items-center justify-center p-4 border-t border-gray-200">
                <?php if ( $permission['addCust'] ): ?>
                    <button type="button" id="btnSaveNewCust" data-id="0" class="inline-block align-middle rounded px-6 py-2.5 ml-1 bg-gray-800 text-white font-medium text-xs leading-tight uppercase shadow-md hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
...
$script = <<<JS
    let e = {
        ...
        btn_add_cust: $('#btnSaveNewCust'),
        loader_add_cust: $('#modalLoaderAdd'),
        input_name_cust: $('input[name=\'new-cust-name\']'),
        input_address_cust: $('input[name=\'new-cust-address\']'),
    ...
    function getCustomer(){
        $.ajax({
            url: '/frontend/web/projects/admin-customer-view',
            type: 'post',
            dataType: 'html',
            data: 1,
            beforeSend: function (){
    ...
    e.btn_add_cust.on('click', function (){
        ...
        let address = e.input_address_cust.val();
        $.ajax({
            url: '/frontend/web/projects/add-customer',
            type: 'post',
            dataType: 'html',
    ...
    e.container.on('click', 'div[name^=\'cust_view_\']', function (){
        e.btn_modal_update_cust.trigger('click');
        current_id = $(this).attr('data-cust');
        $.ajax({
            url: '/frontend/web/projects/admin-customer-view-form',
            type: 'post',
            dataType: 'html',
            data: { 'id': current_id },
            beforeSend: function (){
    ...
    e.btn_update_data.on('click', function (){
        let name = $('input[name=\'update_cust_name\']').val();
        let address = $('input[name=\'update_cust_address\']').val();
        $.ajax({
            url: '/frontend/web/projects/update-customer',
            type: 'post',
            dataType: 'html',
            data: { 'id': current_id, 'name': name, 'address': address },
            beforeSend: function (){
                e.container_current_cust.hide().html("");
    ...
    e.btn_delete_data.on('click', function (){
        $.ajax({
            url: '/frontend/web/projects/delete-customer',
            type: 'post',
            dataType: 'html',
            data: { 'id': current_id },
            beforeSend: function (){
