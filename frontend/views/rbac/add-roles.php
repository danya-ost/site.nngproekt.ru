...
<div class="container h-auto pb-24">
    ...
    <div class="grid grid-cols-2 grid-rows-[auto_auto] gap-10 md:gap-x-28 mt-10">
        ...
        <div id="set_ai" class="bg-white border border-solid border-stone-300 text-green-500"></div>
    </div>
...
$script = <<<JS
    ...
    $('#btn_save').on('click', function (){
        ...
        $.ajax({
            url: '/frontend/web/rbac/add-roles',
            type: 'post',
            dataType: 'html',
            data: { 'name': $('input[name=\'name\']').val(), 'description': $('input[name=\'description\']').val(), 'set_ai': set_ai },
            beforeSend: function (){
                $('#btn_save').hide();
                $('#loader').show();  
            },
             success: function (response){
                getLogs('rbac', 'add', 'adding new role to database: '+$('select[name=\'name\']').val(), 1, false);
                window.location.href = '/frontend/web/rbac/index';
            },
            error: function (response){
                getLogs('rbac', 'add', 'adding new role to database: '+$('select[name=\'name\']').val(), 0, response['responseText']);
