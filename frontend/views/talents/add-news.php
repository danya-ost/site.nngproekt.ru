...
/* @var frontend\controllers\TalentsController $category_list */
/* @var frontend\controllers\TalentsController $department_list */
...
<div class="container">
    ...
    <form id="newsContainer" class="w-full h-auto">
        ...
        <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 grid-rows-1 gap-5">
            <div>
                <label for="category_id" class="font-light text-xs text-stone-500">
                    Категория
                </label>
            ...
            <div>
                <label for="department_id" class="font-light text-xs text-stone-500">
                    Подразделение
                </label>
        ...
        <div id="covers" class="mt-5 grid grid-cols-1 sm:grid-cols-2 grid-rows-1 gap-5">
            <div>
                ...
                <div id="imageView" class="min-h-[350px] max-h-full w-full bg-cover bg-center bg-no-repeat">
            ...
            <div>
                ...
                <video id="videoView" src="" height="350" controls class="w-full" style="height: 350px;">
                    <source id="videoViewSrc" src="" type="video/mp4">
                </video>
            </div>
        </div>

        <div class="py-10">
            <button type="button" id="btnImgAdd" data-bs-toggle="modal" data-bs-target="#imageModal" class="inline-block align-middle rounded mt-1 px-6 py-2.5 ml-1 bg-gray-800 text-white font-medium text-xs leading-tight uppercase shadow-md hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>
                <span class="ml-1 inline-block align-middle">Добавить обложку</span>
            </button>
            ...
            <button type="button" id="onFixed" class="inline-block align-middle rounded mt-1 px-6 py-2.5 ml-1 bg-gray-800 text-white font-medium text-xs leading-tight uppercase shadow-md hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013.361-6.867 8.21 8.21 0 003 2.48z" />
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 18a3.75 3.75 0 00.495-7.467 5.99 5.99 0 00-1.925 3.546 5.974 5.974 0 01-2.133-1A3.75 3.75 0 0012 18z" />
                </svg>
                <span class="ml-1 inline-block align-middle">Закрепить</span>
            </button>
            <button type="button" id="offFixed" class="inline-block align-middle rounded mt-1 px-6 py-2.5 ml-1 bg-gray-200 text-gray-700 font-medium text-xs leading-tight uppercase shadow-md hover:bg-gray-300 hover:shadow-lg focus:bg-gray-300 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-400 active:shadow-lg transition duration-150 ease-in-out">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013.361-6.867 8.21 8.21 0 003 2.48z" />
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 18a3.75 3.75 0 00.495-7.467 5.99 5.99 0 00-1.925 3.546 5.974 5.974 0 01-2.133-1A3.75 3.75 0 0012 18z" />
                </svg>
                <span class="ml-1 inline-block align-middle">Открепить</span>
            </button>
    ...
    <div id="newsView" class="w-full h-auto"></div>
...
$script = <<<JS
    ...
    function getItsSending(){ 
        let params_lock = 0;
        for (let key in its_sending){
                if( its_sending[key] ){
                    params_lock++
                }
            }
        if( params_lock >= 3 ){ elements.btnSave.show(); } else{ elements.btnSave.hide(); } 
        console.log(params_lock);
    }
    
    elements.loader.hide();
    elements.off_fixed.hide();
    elements.btn_img_reload_modal.hide();
    elements.loader_image.hide();
    elements.btn_video_reload_modal.hide();
    elements.loader_video.hide();
    elements.covers.hide();
    elements.btnSave.hide();
    
    elements.on_fixed.on('click', function (){
        values.fixed.val(1); 
        $(this).hide();
        elements.off_fixed.show();
    });
    
    elements.off_fixed.on('click', function (){
        values.fixed.val(0); 
        $(this).hide();
        elements.on_fixed.show();
    });
    ...
    values.title.bind('input, keyup', function (){
        let value = $(this).val();
        if( value.length > 0 ){
            its_sending.params_01 = true;
            getItsSending();
        } else{
            its_sending.params_01 = false;
            getItsSending();
        }
    });
    
    values.annotation.bind('input, keyup', function (){
        let value = $(this).val();
        if( value.length > 0 ){
            its_sending.params_02 = true;
            getItsSending();
        } else{
            its_sending.params_02 = false;
            getItsSending();
        }
    });
    ...
    elements.btnSave.on('click', function (){
        $.ajax({
            url: '/frontend/web/talents/add-news',
            type: 'post',
            dataType: 'html',
            data: {
                'title': values.title.val(),
                'department_id': values.department_id.val(),
                'category_id': values.category_id.val(),
                'annotation': values.annotation.val(),
                'content': tinymce.get("content").getContent(),
                'image_src': values.image.val(),
                'video_src': values.video.val(),
                'status': values.fixed.val()
            },
            beforeSend: function (){
            ...
            success: function (response){
                // elements.container_adding.html(response).show();
                // elements.loader.hide();
                // $('html, body').animate({ scrollTop: 0 }, 'slow');
                location.href = '/frontend/web/talents/view-news?n='+response;
            },
            error: function (response){
                elements.loader.hide();
