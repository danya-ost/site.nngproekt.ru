...
/* @var frontend\controllers\TalentsController $lastNews */
/* @var frontend\controllers\TalentsController $permission */

use yii\helpers\Url;

$this->title = 'Таланты ННГП';
?>

<div class="container h-auto">

    <div class="w-full ha-auto">
        <h1 class="font-bold text-4xl uppercase">ПОСТЫ "Таланты"</h1>
    </div>

    <?php if ( $permission['addTalents'] || $permission['updateTalentsCategory'] ): ?>
        <div class="relative rounded overflow-hidden my-5 bg-white shadow-sm hover:shadow-lg px-2 py-3 border border-solid border-stone-200 duration-200 ease-in-out transition-al pb-10 sm:pb-3">
            ...
            <?php if ( $permission['addTalents'] ): ?>
                <a href="<?= Url::to(['/talents/add-news-form']) ?>" class="block sm:inline-block align-middle text-center rounded px-6 py-2.5 ml-1 mt-1 md:mt-0 bg-stone-300 text-white font-medium text-xs leading-tight uppercase hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                    ...
                    <span class="ml-1 inline-block align-middle">Добавить пост</span>
                </a>
            <?php endif; ?>
            <?php if ( $permission['updateTalentsCategory'] ): ?>
                <a href="<?= Url::to(['/talents/admin-category']) ?>" type="button" class="block sm:inline-block align-middle text-center rounded px-6 py-2.5 ml-1 mt-1 md:mt-0 bg-stone-300 text-white font-medium text-xs leading-tight uppercase hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block align-middle">
...
$script = <<<JS
    ...
    function getNews(last_id){
        $.ajax({
            url: '/frontend/web/talents/index-view-news',
            type: 'post',
            dataType: 'html',
    ...
    elements.search_input.bind('input, keyup', function (){
        ...
        if( value.length > 3 ){
            query_ajax.abort();
            query_ajax = $.ajax({
                url: '/frontend/web/talents/index-search',
                type: 'post',
                dataType: 'html',
                data: { 'value': value },
                beforeSend: function (){
